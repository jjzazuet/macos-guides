 ...
 *
 * Disassembly of SSDT-2-PegSsdt.aml, Sun Dec  9 00:55:39 2018
 *
 * Original Table Header:
 *     Signature        "SSDT"
 *     Length           0x00002896 (10390)
 *     Revision         0x02
 *     Checksum         0xDE
 *     OEM ID           "PegSsd"
...
DefinitionBlock ("", "SSDT", 2, "PegSsd", "PegSsdt", 0x00001000)
{
    ...
    External (CKM0, UnknownObj)    // (from opcode)
    External (CKM1, UnknownObj)    // (from opcode)
    External (CKM2, UnknownObj)    // (from opcode)
    External (CPEX, UnknownObj)    // (from opcode)
    External (DLAL, UnknownObj)    // (from opcode)
    External (DLAO, UnknownObj)    // (from opcode)
    ...
    External (GSCE, UnknownObj)    // (from opcode)
    ...
    Scope (\_SB.PCI0)
    {
        ...
        Method (PGON, 1, Serialized)
        {
            ...
            If (LEqual (Arg0, Zero))
            {
                Store (CKM0, Index (SCLK, One))
            }
            ElseIf (LEqual (Arg0, One))
            {
                Store (CKM1, Index (SCLK, One))
            }
            ElseIf (LEqual (Arg0, 0x02))
            {
                Store (CKM2, Index (SCLK, One))
            }
        ...
        Method (CLSP, 1, Serialized)
        {
            ...
            If (LAnd (LNotEqual (Arg0, One), LAnd (LNotEqual (Arg0, 0x02), LNotEqual (Arg0, 0x03))))
            {
                Return (Zero)
            }
